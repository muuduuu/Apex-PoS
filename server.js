...
// GET /api/reports/sales-excel - Generate Excel report for a date range
app.get('/api/reports/sales-excel', authenticateJWT, async (req, res) => {
  try {
    ...
    const summaryData = [
      ...
      [],
      ['Sale #', 'Date', 'Time', 'Cashier', 'Contractor', 'Status', 'Subtotal (KWD)', 'Discount', 'Total (KWD)', 'Payment Method', 'Reference', 'Notes']
    ];
    ...
    salesResult.rows.forEach(s => {
      ...
      summaryData.push([
        ...
        s.contractor_name || '',
        s.status,
        Number(s.subtotal).toFixed(3),
        s.discount_amount ? `${Number(s.discount_amount).toFixed(3)} (${s.discount_percentage}%)` : '0.000',
        Number(s.total_amount).toFixed(3),
        s.payment_method,
        s.knet_reference || s.cheque_number || '',
        s.notes || ''
      ]);
    ...
    const totalRevenue = salesResult.rows.reduce((sum, s) => sum + Number(s.total_amount), 0);
    const totalDiscount = salesResult.rows.reduce((sum, s) => sum + Number(s.discount_amount || 0), 0);

    summaryData.push([]);
    summaryData.push(['SUMMARY']);
    summaryData.push(['Total Sales:', totalSales]);
    summaryData.push(['Total Revenue (KWD):', totalRevenue.toFixed(3)]);
    summaryData.push(['Total Discounts (KWD):', totalDiscount.toFixed(3)]);
    ...
    const itemsData = [
      ...
      [`Period: ${start_date} to ${end_date}`],
      [],
      ['Sale #', 'Item Name (EN)', 'Item Name (AR)', 'Quantity', 'Unit Price (KWD)', 'Line Total (KWD)']
    ];
    ...
    salesResult.rows.forEach(s => {
      if (!paymentBreakdown[s.payment_method]) {
        paymentBreakdown[s.payment_method] = { count: 0, total: 0 };
      }
      paymentBreakdown[s.payment_method].count++;
      paymentBreakdown[s.payment_method].total += Number(s.total_amount);
    });
    ...
    const paymentData = [
      ...
      [`Period: ${start_date} to ${end_date}`],
      [],
      ['Payment Method', 'Number of Sales', 'Total Amount (KWD)']
    ];
    ...
    ws3['!cols'] = [{ wch: 20 }, { wch: 18 }, { wch: 20 }];

    XLSX.utils.book_append_sheet(wb, ws3, 'Payment Breakdown');
